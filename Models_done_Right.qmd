...
conditional_Plot_N_P_df<-NewData_N_P_brms%>%
  ...
  reframe(tidybayes::median_qi(.linpred,.width=c(0.80))) %>%
    mutate(N_P = N_P*df_back_to_raw %>% 
             ...
             dplyr::filter(metrics == "N_P") %>% 
             pull(avg),
           y = case_when(Box == "Adriatic" ~ 2 * mean(y) - y,
                         T ~ y),
           ymin = case_when(Box == "Adriatic" ~ 2 * mean(ymin) - ymin,
                         T ~ ymin),
           ymax = case_when(Box == "Adriatic" ~ 2 * mean(ymax) - ymax,
                         T ~ ymin)) 
...
ggsave("Outputs/plot_all_variable_per_Box.png", height = 5.25, width = 10, unit = "in", dpi = 300)
```
