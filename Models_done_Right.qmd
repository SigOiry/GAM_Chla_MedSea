...
df_wide_all <- df_wide %>%
  ...
  mutate(Box = case_when(Box == "Lyon" ~ "Box 1",
                         Box == "Adriatic" ~ "Box 2",
                         T ~ "Box 3"))
  # dplyr::filter(MLD <400)


ggplot(df_wide_all, aes(x = Date, y = MLD, color = Box))+
  geom_line()
...
fit_all <- brm(
  ...
  file = "Data/Model/A_Model_Unique_STD_all_5000_Student_all_New_Extraction_NoNutriRow_cleaned_final"
)
...
conditional_Plot_N_P_df<-NewData_N_P_brms%>%
  ...
  reframe(tidybayes::median_qi(.linpred,.width=c(0.80))) %>%
    mutate(N_P = N_P*df_back_to_raw %>% 
             ...
             dplyr::filter(metrics == "N_P") %>% 
             pull(avg),
           y = case_when(Box == "Box 2" ~ 2 * mean(y) - y,
                         T ~ y),
           ymin = case_when(Box == "Box 2" ~ 2 * mean(ymin) - ymin,
                         T ~ ymin),
           ymax = case_when(Box == "Box 2" ~ 2 * mean(ymax) - ymax,
                         T ~ ymax)
           ) 
...
combined_plot <- (conditional_Plot_SST/ conditional_Plot_N_P) +
  plot_layout(guides = "collect") & 
  theme(aspect.ratio = 1) &
...
ggsave("Outputs/plot_all_variable_per_Box.png", height = 8, width = 10, unit = "in", dpi = 300)
```
