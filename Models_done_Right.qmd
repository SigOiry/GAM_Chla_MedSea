...
fit_all <- brm(
  ...
  file = "Data/Model/A_Model_Unique_STD_all_5000_Student_all_New_Extraction_NoNutriRow"
)
...
geom_ribbon(aes(ymax = ymax,ymin=ymin, fill = Box))+
geom_line(aes(color = Box),size = 1)+
        scale_color_manual(values = colscale)+
        scale_fill_manual(values = colscale_rib)+
...
conditional_Plot_N_P <- conditional_Plot_N_P_df %>% 
  ...
  ggplot(aes(x = N_P,y=y)) +
geom_ribbon(aes(ymax = ymax,ymin=ymin, fill = Box))+
geom_line(aes(colour=Box), size =1)+
  facet_wrap(~Box, scales = "free_x")+
...
geom_ribbon(aes(ymax = ymax,ymin=ymin, fill = Box))+
geom_line(aes(colour=Box) ,linewidth = 1)+
  facet_wrap(~Box, scales = "free_x")+
    ylab("Chla Concentration (mg.m⁻³)")+
...
ggsave("Outputs/plot_all_variable_per_Box.png", height = 7.5, width = 10, unit = "in", dpi = 300)
```
