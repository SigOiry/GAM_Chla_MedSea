...
df <- df_chla %>%
  ...
  ungroup() %>% 
  mutate(Box = case_when(Box == "Lyon" ~ "Box 1",
                         Box == "Adriatic" ~ "Box 2",
                         T ~ "Box 3"))
...
colscale <- c("Box 2" = "#4F8EB1","Box 1" = "#F6F9A0","Box 3" = "#223F8D")
...
(plot_seas_Chla <- df %>% 
  filter(Metric %in% c("GeoMean_Chla")) %>% 
  mutate(
    day      = yday(Date),
    # bin      = (day - 1) %/% bin_size * bin_size + bin_size/2,
    Metric = case_when(
      Metric == "GeoMean_Chla" ~ "bold('A) Chlorophyll-a'~(mg%.%m^{-3}))",
      Metric == "MLD" ~ "bold('MLD (m)')",
      Metric == "NPratio" ~ "NO3:PO4",
      Metric == "TotalNutrient" ~ "Total Nutrient",
      Metric == "SST" ~ "bold('SST (°C)')",
      TRUE ~ Metric
    ),
    day = case_when(day == 61 ~ 60,
                     day == 92 ~ 91,
                     day == 122 ~ 121,
                     day == 153 ~ 152,
                     day == 183 ~ 182,
  ...
  geom_linerange(aes(ymin = p25, ymax = p75), linewidth = .8) +
     ...
     # keep your smooth
  facet_wrap(~Metric , ncol = 3,scales = "free",labeller = label_parsed) +
  ...
  scale_y_continuous(
    breaks = round(seq(0, 1.1, length.out = 5),2)
  ) +
  labs(x = NULL, y = NULL) +
  theme_Bede() +
...
(plot_seas_SST <- df %>% 
  ...
  mutate(
    ...
    # bin      = (day - 1) %/% bin_size * bin_size + bin_size/2,
    Metric = case_when(
      Metric == "GeoMean_Chla" ~ "bold('Chlorophyll-a'~(mg%.%m^{-3}))",
      Metric == "MLD" ~ "bold('MLD (m)')",
      Metric == "NPratio" ~ "NO3:PO4",
      Metric == "TotalNutrient" ~ "Total Nutrient",
      Metric == "SST" ~ "B) SST (°C)",
  ...
  scale_y_continuous(
    breaks = round(seq(12.5, 30, length.out = 5),2),
    limits = c(12.5,30)
  ) +
  labs(x = NULL, y = NULL) +
  theme_Bede() +
...
(plot_seas_MLD <- df %>% 
  ...
  scale_y_continuous(
    breaks = round(seq(0, 520, length.out = 5),2),
    # limits = c(-100,550)
  ) +
  labs(x = NULL, y = NULL) +
  theme_Bede() +
...
(plot_seas_Nutri <- df %>% 
  filter(Metric %in% c("TotalNutrient")) %>% 
  mutate(
    day      = yday(Date),
    # bin      = (day - 1) %/% bin_size * bin_size + bin_size/2,
    Metric = case_when(
      ...
      Metric == "MLD" ~ "bold('MLD (m)')",
      Metric == "NPratio" ~ "NO3:PO4",
      Metric == "TotalNutrient" ~ "bold('D)  Sum of Nutrients'~(mmol%.%m^{-3}))",
      Metric == "SST" ~ "bold('SST (°C)')",
      TRUE ~ Metric
    ),
    day = case_when(day == 61 ~ 60,
  ...
  scale_y_continuous(
    breaks = round(seq(0, 5, length.out = 5),2),
    limits = c(0,5)
  ) +
  labs(x = NULL, y = NULL) +
  theme_Bede() +
...
(plot_seas_NP <- df %>% 
  ...
  mutate(
    ...
    Metric = case_when(
      Metric == "GeoMean_Chla" ~ "bold('Chlorophyll-a'~(mg%.%m^{-3}))",
      Metric == "MLD" ~ "bold('MLD (m)')",
      Metric == "NPratio" ~ "E) NO3:PO4",
      Metric == "TotalNutrient" ~ "E) Sum of Nutrients",
      Metric == "SST" ~ "bold('SST (°C)')",
      TRUE ~ Metric
    ),
    day = case_when(day == 61 ~ 60,
  ...
  scale_y_continuous(
    breaks = round(seq(0, 300, length.out = 5),2),
    limits = c(0,300)
  ) +
  labs(x = NULL, y = NULL) +
  theme_Bede() +
...
combined_plot <- (plot_seas_Chla + plot_seas_SST + plot_seas_MLD) / (plot_seas_Nutri + plot_seas_NP) + 
...
ggsave("Outputs/Seasonality_variables.png",combined_plot, dpi = 300)
